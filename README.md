...
docker run  -p 80:80 -p 3234:3234 -d -v ~/Sites:/www mysite
docker start -p 80:80 -d -v ~/Sites:/www

docker run  -p 80:80  -p 8089:8089 -d mysite
(加上-d是容器在后端静默运行  不加-d会看到运行的日志 但是一旦ctrl+c 整个容器也停止了)
